...
public class Configuration {
    ...
    public Character getChar(final String path) {
        ...
        try {
            ...
            throw new RuntimeException(String.format("任务读取配置文件出错. 因为配置文件路径[%s] 值非法，期望是字符类型: %s. 请检查您的配置并作出修改.", path,
                    e.getMessage()));
        }
    }
    ...
    public Boolean getBool(final String path) {
        ...
        if (null == result) {
            ...
            throw new RuntimeException(String.format("您提供的配置信息有误，因为从[%s]获取的值[%s]无法转换为bool类型. 请检查源表的配置并且做出相应的修改.",
                    path, result));
        }
    ...
    public Integer getInt(final String path) {
        ...
        try {
            ...
            throw new RuntimeException(String.format("任务读取配置文件出错. 配置文件路径[%s] 值非法, 期望是整数类型: %s. 请检查您的配置并作出修改.", path,
                    e.getMessage()));
        }
    }
    ...
    public Double getDouble(final String path) {
        ...
        try {
            ...
            throw new RuntimeException(String.format("任务读取配置文件出错. 配置文件路径[%s] 值非法, 期望是浮点类型: %s. 请检查您的配置并作出修改.", path,
                    e.getMessage()));
        }
    }
    ...
    public Object remove(final String path) {
        ...
        if (null == result) {
            throw new RuntimeException(String.format("配置文件对应Key[%s]并不存在，该情况是代码编程错误. 请联系开发团队.", path));
        }
    ...
    private void setObject(final String path, final Object object) {
        ...
        throw new RuntimeException(String.format("值[%s]无法适配您提供[%s]， 该异常代表系统编程错误, 请联系开发团队.",
    ...
    Object buildObject(final List<String> paths, final Object object) {
        if (null == paths) {
            throw new RuntimeException("Path不能为null，该异常代表系统编程错误, 请联系开发团队");
        }
        ...
        for (int i = paths.size() - 1; i >= 0; i--) {
            ...
            throw new RuntimeException(String.format(
                    "路径[%s]出现非法值类型[%s]，该异常代表系统编程错误, 请联系开发团队.",
    ...
    Object setObjectRecursive(Object current, final List<String> paths,
        ...
        throw new RuntimeException("该异常代表系统编程错误, 请联系开发团队.");
    }
    ...
    private Object findObjectInList(final Object target, final String each) {
        ...
        if (!StringUtils.isNumeric(index)) {
            throw new IllegalArgumentException(
                    String.format(
                            "系统编程错误，列表下标必须为数字类型，但该节点发现实际类型是[%s] ，该异常代表系统编程错误, 请联系开发团队.",
                            index));
        }
    ...
    private Configuration(final String json) {
        try {
            ...
            throw new RuntimeException(String.format("配置信息错误. 您提供的配置信息不是合法的JSON格式: %s . 请按照标准json格式提供配置信息. ", e.getMessage()));
        }
    }
